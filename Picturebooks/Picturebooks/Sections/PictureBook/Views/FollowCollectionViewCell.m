...
#define SOUND_RECORD_LIMIT 1200
...
//        [self.recordButton setImage:[UIImage imageNamed:@"record_sel"] forState:UIControlStateNormal];
        ...
        if ([self.delegate respondsToSelector:@selector(followCollectionView:recordStatus:)]) {
            [self.delegate followCollectionView:self recordStatus:btn.selected];
        }
...
//    }
    ...
    if (IS_IPHONE4) {
        self.content.frame = CGRectMake(0, 0, 180, [AppTools heightForAttributeContent:conversation fontoOfText:15 width:180 lineSpace:5]);
        //根据content高度，给scrollview核心属性赋值
        self.contentScroll.contentSize = CGSizeMake(0, self.content.frame.size.height);
        self.contentScroll.contentOffset = CGPointMake(0, 0);
//        self.scrollBar.frame = CGRectMake(0, 0, 8, 95 / self.content.frame.size.height * 95);
        ...
        if (IS_IPHONE5()) {
            self.content.frame = CGRectMake(0, 0, 220, [AppTools heightForAttributeContent:conversation fontoOfText:15 width:220 lineSpace:5]);
            //根据content高度，给scrollview核心属性赋值
            self.contentScroll.contentSize = CGSizeMake(0, self.content.frame.size.height);
            self.contentScroll.contentOffset = CGPointMake(0, 0);
//            self.scrollBar.frame = CGRectMake(0, 0, 8, 145 / self.content.frame.size.height * 145);
            ...
            self.content.frame = CGRectMake(0, 0, 250, [AppTools heightForAttributeContent:conversation fontoOfText:15 width:250 lineSpace:5]);
            //根据content高度，给scrollview核心属性赋值
            self.contentScroll.contentSize = CGSizeMake(0, self.content.frame.size.height);
            self.contentScroll.contentOffset = CGPointMake(0, 0);
