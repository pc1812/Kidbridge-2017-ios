...
- (NSArray *)nameArr{
    if (!_nameArr) {
//        _nameArr = [NSMutableArray arrayWithObjects:@"蓝思指数",@"故事梗概", @"绘本感悟", @"适龄", @"关键词",nil];
...
- (void)loadData{
    ...
    [[HttpManager sharedManager] POST:Picturebook_detail parame:parame sucess:^(id success) {
        ...
        if ([success[@"event"] isEqualToString:@"SUCCESS"]) {
            ...
            if ([Global isNullOrEmpty:model.lexile]) {
                self.dataArr = @[model.outline, model.feeling, ageStr, keyStr];
                _nameArr = @[@"故事梗概", @"绘本感悟", @"适龄", @"关键词"];
            } else {
                self.dataArr = @[model.lexile,model.outline, model.feeling, ageStr, keyStr];
                _nameArr = @[@"蓝思指数",@"故事梗概", @"绘本感悟", @"适龄", @"关键词"];
            }
        ...
        if ([success[@"event"] isEqualToString:@"SUCCESS"]) {
            ...
            [[HttpManager sharedManager] POST:Repeat_token parame:parame_ sucess:^(id success) {
                ...
                if ([[success objectForKey:@"event"] isEqualToString:@"SUCCESS"]) {
                    ...
                    NSLog(@" 01 event = %@, describe = %@", [success objectForKey:@"event"], [success objectForKey:@"describe"]);
                }
...
//                }];
                ...
                NSLog(@" 02 event = %@, describe = %@", [success objectForKey:@"event"], [success objectForKey:@"describe"]);
            }
        } failure:^(NSError *error) {
