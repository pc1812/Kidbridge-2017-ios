...
@interface PicWithReadViewController ()<UITableViewDelegate, UITableViewDataSource, SDCycleScrollViewDelegate,SRActionSheetDelegate, UITextFieldDelegate, LGAudioPlayerDelegate, AVAudioPlayerDelegate, UIWebViewDelegate,WKNavigationDelegate>
...
//    playBtn = [UIButton new];
    ...
    // 解锁绘本
    if (self.belong == -1) {
        [self requestUnLockBook];
    }
    
//    _webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 1)];
    _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 1)];
...
//    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/book/richtext/%ld", URL_api, self.picId]];
//    NSURLRequest *request = [[NSURLRequest alloc] initWithURL:url cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:2.0];
...
/** 解锁免费的绘本 */
- (void)requestUnLockBook
{
    NSMutableDictionary *parame_ = [HttpManager necessaryParameterDictionary];
    [parame_ setObject:PICTURE_UNLOCK forKey:@"uri"];
    [parame_ setObject:[HttpManager getAddSaltMD5Sign:parame_] forKey:@"sign"];
    [parame_ setObject:@(self.picId) forKey:@"id"];
    
    [[HttpManager sharedManager] POST:PICTURE_UNLOCK parame:parame_ sucess:^(id success) {
