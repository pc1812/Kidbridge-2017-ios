...
{
    FollowCollectionViewCell *currentCell;
}
...
- (void)countDownAction{
    ...
    if (_secondCountDown == 0) {
        ...
        // 停止录音
        [currentCell confirmRecordVoice];
...
//录音按钮触发代理方法
- (void)followCollectionView:(FollowCollectionViewCell *)cell recordStatus:(NSInteger)status {
    currentCell = cell;
    NSLog(@"当前的 cell:%@",currentCell);
    if (status) {
        self.myCollectionView.scrollEnabled = NO;
        [Global showWithView:self.view withText:@"跟读录制开始～"];
    }else{
