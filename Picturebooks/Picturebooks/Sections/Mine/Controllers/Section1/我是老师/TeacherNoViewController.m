...
#import "TeacherNoViewController.h"
...
@interface TeacherNoViewController ()<UICollectionViewDataSource, UICollectionViewDelegate>
...
@implementation TeacherNoViewController
...
- (void)loadData{
    ...
    [parame setObject:@(0) forKey:@"status"];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    CourseModel *model = self.dataSource[indexPath.row];
    if (model.status == 0) {
        [Global showWithView:self.view withText:@"该课程未到开课时间，暂不能查看用户打卡记录"];
    }else{
