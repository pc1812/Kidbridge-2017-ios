...
#pragma mark - 第一个输入框
    ...
    UITapGestureRecognizer *preLabtapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(preLabtapGesture)];
    [self.preLabel addGestureRecognizer:preLabtapGesture];
...
//                    NSLog(@"登录成功:%@",success);
                    ...
                    if ([[success objectForKey:@"event"] isEqualToString:@"SUCCESS"]) {
                        ...
                        [[NSUserDefaults standardUserDefaults] setObject:[Qiniu_host stringByAppendingString:success[@"data"][@"head"]] forKey:@"User_headimage"];
                        [[NSUserDefaults standardUserDefaults] synchronize];
