...
- (void)viewDidLoad {
    ...
    [Global showWithView:self.view withText:@"微信授权成功，请继续完成手机号码绑定"];
...
#pragma mark - 第一个输入框
    ...
    [firstShadow mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(headImageView.mas_bottom).offset(70 * PROPORTION);
    }];
...
#pragma mark - 第三个输入框
    ...
    [thirdShadow mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(secondShadow.mas_bottom).offset(15);
        make.width.mas_equalTo(275);
        make.height.mas_equalTo(44);
        make.left.mas_equalTo((SCREEN_WIDTH - 275) / 2);
    }];
...
#pragma mark - 下部分视图
    ...
    [forthShadow addSubview:bindingButton];
    ...
    bindingButton.titleLabel.font = [UIFont boldSystemFontOfSize:15];
    bindingButton.backgroundColor = RGBHex(0x14d02f);
    bindingButton.layer.masksToBounds = YES;
    bindingButton.layer.cornerRadius = 22;
    [bindingButton addTarget:self action:@selector(bindingBtnAct:) forControlEvents:UIControlEventTouchUpInside];
...
//绑定按钮触发方法
- (void)bindingBtnAct:(UIButton *)btn{
    ...
    if ([Global isNullOrEmpty:self.phoneNum.text]) {
        ...
        if ([AppTools isNotPhoneNumber:self.phoneNum.text]) {
            ...
            [parame setObject:self.token forKey:@"bind"];
            ...
            [[HttpManager sharedManager] POST:User_register parame:parame sucess:^(id success) {
                ...
                if ([[success objectForKey:@"event"] isEqualToString:@"SUCCESS"]) {
                    ...
                    [Global showWithView:self.view withText:@"绑定成功！即将登陆"];
