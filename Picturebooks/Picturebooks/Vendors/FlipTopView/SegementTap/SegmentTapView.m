...
-(instancetype)initWithFrame:(CGRect)frame withDataArray:(NSArray *)dataArray withFont:(CGFloat)font {
    ...
    if (self) {
        ...
        self.lineColor = [Global convertHexToRGB:@"8fe69c"];
...
{
    float width = (self.frame.size.width-_dataArray.count*1) / _dataArray.count;
    ...
    for (int i = 0 ; i < _dataArray.count ; i++) {
        ...
        if (i!=_dataArray.count-1) {
            UIView *lineView=[[UIView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(button.frame), 0, 1, self.frame.size.height)];
            lineView.backgroundColor=[Global convertHexToRGB:@"eeeeee"];
...
//    self.lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH / index - SCREEN_WIDTH / index / 4 * index, self.frame.size.height-2, SCREEN_WIDTH / index / 2, 2)];
...
-(void)tapAction:(id)sender{
    ...
    [UIView animateWithDuration:0.2 animations:^{
       //self.lineImageView.frame = CGRectMake(button.frame.origin.x, self.frame.size.height-2, button.frame.size.width, 2);
        self.lineImageView.frame = CGRectMake(button.frame.origin.x + button.frame.size.width / 4, self.frame.size.height-2, button.frame.size.width / 2, 2);
    }];
        ...
        if (index != subButton.tag) {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                //self.lineImageView.frame = CGRectMake(subButton.frame.origin.x, self.frame.size.height - 2, subButton.frame.size.width , 2);
                  self.lineImageView.frame = CGRectMake(subButton.frame.origin.x + subButton.frame.size.width / 4, self.frame.size.height-2, subButton.frame.size.width / 2, 2);
            }];
        }
    }
}
