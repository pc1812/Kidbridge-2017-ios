...
//私有方法，只允许在类的内部调用。在.h里不声明，直接在.m里实现
- (NSData *)audioDataFromURLString:(NSString *)URLString atIndex:(NSUInteger)index{
    ...
    if (audioData) {
        objc_setAssociatedObject(audioData, &kXMNAudioDataKey, [NSString stringWithFormat:@"%@_%ld", URLString, (long)index], OBJC_ASSOCIATION_COPY);
    }
...
- (void)playAudioWithData:(NSData *)audioData {
    ...
    if (![[NSString stringWithFormat:@"%@_%ld", self.URLString, (long)self.index] isEqualToString:audioURLString]) {
        return;
    }
